...
def calculate_cost_for_components(square_footage, primary_energy_source, include_tes, include_chilled_beams):
    ...
    tes_cost = (square_footage / 2000 * TES_COST_FOR_2000SF_HOME_5DAYS) if include_tes and primary_energy_source == "Solar Thermal" else 0
...
def main():
    ...
    primary_energy_source = st.radio("Select Primary Energy Source", ["Solar PV", "Solar Thermal"])
    ...
    include_tes = primary_energy_source == "Solar Thermal"
    include_chilled_beams = st.checkbox("Include Chilled Beams")

    if st.button("Calculate Energy Needs and Costs"):
        net_energy = calculate_net_energy(construction_type, primary_energy_source, include_tes, stirling_chiller, stirling_generator, excess_capacity)
        if net_energy >= 0:
        ...
        solar_pv_cost, solar_thermal_cost, tes_cost, chilled_beam_cost = calculate_cost_for_components(square_footage, primary_energy_source, include_tes, include_chilled_beams)
        ...
        st.write(f"Solar Thermal Cost: ${solar_thermal_cost:.2f}" if primary_energy_source == "Solar Thermal" else "Solar Thermal not selected as primary energy source.")
